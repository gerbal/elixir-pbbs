defmodule PBBS.Geometry.ConvexHull.Parallel do
  ...
  def hsplit(index, points, {p1, p1_index}, {p2, p2_index}) do
    ...
    if length(packed) < 2 do
      ...
      pm =
        Enum.max_by(zipped, fn {_point, cross} -> cross end)
        |> elem(0)

      if length(packed) < 1000 do
        hsplit(index * 2, packed, {p1, p1_index}, pm) ++
          hsplit(index * 2 + 1, packed, pm, {p2, p2_index})
      else
        ...
        [first, second] =
          Task.await_many([
            Task.async(fn ->
              pts =
                Keyword.get(
                  :ets.lookup(:ch, String.to_atom("pts#{index}")),
                  String.to_atom("pts#{index}")
                )
