  ...
  def run_benchmark() do
    ...
    impl_map =
      Enum.flat_map(plist, fn p ->
        [
          {"parallel;p=#{p};trigrams",
           fn -> PBBS.Strings.SuffixArray.Parallel.suffix_array(trigrams, p) end},
          {"parallel;p=#{p};dna",
           fn -> PBBS.Strings.SuffixArray.Parallel.suffix_array(dna, p) end}
        ]
      end)
      |> Map.new()
      |> Map.put("serial;trigrams", fn ->
        PBBS.Strings.SuffixArray.Sequential.suffix_array(trigrams)
      end)
